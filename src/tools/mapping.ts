...
export async function handleCreateMapping(args: any) {
...
function generateMappingCode(entities: any[], generateExample: boolean, includeValidation: boolean) {
  ...
  entities.forEach((entity, entityIndex) => {
    ...
    entity.properties.forEach((prop: any, propIndex: number) => {
    ...
    if (entity.relations && entity.relations.length > 0) {
      mappingCode += `,\n    relations: {\n`;
      entity.relations.forEach((rel: any, relIndex: number) => {
...
function generateMappingValidation(entities: any[]) {
...
function generateMappingExample(entities: any[]) {
  ...
  if (firstEntity) {
    ...
    firstEntity.properties.forEach((prop: any, index: number) => {
  ...
  if (entities[0]) {
    ...
    entity.properties.forEach((prop: any, index: number) => {
    ...
    if (entity.properties.some((p: any) => p.name.toLowerCase().includes('date'))) {
      const dateProp = entity.properties.find((p: any) => p.name.toLowerCase().includes('date'));
      if (dateProp) {
